...
class HostWaitingActivity : AppCompatActivity() {
        ...
        while (jump < game.users.size) {
            ...
            while (roundCount < game.users.size - jump) {
                ...
                oneRound += (Round(
                    voters,
                    linkedMapOf(
                        GameMethods.opp0 to Opponent(
                            game.users[roundCount]
                        ),
                        GameMethods.opp1 to Opponent(
                            game.users[roundCount + jump]
                        )
                    ),
                    ""
                ))
    ...
    fun getQuestionsForGame(rounds: Int, player_count: Int, selected_category: String){
        ...
        val callback = object:
            Callback<java.util.ArrayList<Question>> {
