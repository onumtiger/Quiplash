        ...
        companion object {
            ...
            fun playerAllocation(appcontext: Context, userid: String){
                if(GameManager.game.playrounds.size>= GameManager.game.activeRound) {
                    ...
                    if (GameManager.game.playrounds.getValue("round${GameManager.game.activeRound}").opponents.getValue(
                            opp0
                        ).userID.equals(userid) || GameManager.game.playrounds.getValue("round${GameManager.game.activeRound}").opponents.getValue(
                            opp1
            ...
            fun setPoints(callback: Callback<Boolean>) {
                val callbackGame = object :
                    ...
                    override fun onTaskComplete(result: Game) {
                        ...
                        if (GameManager.game.playrounds.getValue("round${GameManager.game.activeRound}").opponents.getValue(
                                opp0
                            ).answerScore > GameManager.game.playrounds.getValue(
                                "round${GameManager.game.activeRound}"
                            ).opponents.getValue(opp1).answerScore
                        ) {
                            GameManager.game.playrounds.getValue("round${GameManager.game.activeRound}").opponents.getValue(
                                opp0
